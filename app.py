...
def create_slide_clip(slide_text, video_url, tmpdir, slide_idx, duration=4):
    """Create one video clip: PIL text frame as video (fast on free tier)"""
    out_path = os.path.join(tmpdir, f"slide_{slide_idx:02d}.mp4")

    # Always use PIL frame — fast, no heavy ffmpeg filters
    ...
    if result.returncode != 0:
        # absolute fallback: 1-frame video
        cmd2 = [
            FFMPEG_BIN, "-y",
            "-loop", "1", "-i", img_path,
            "-t", str(duration),
